<script>
	import {current, currentWords, lose, allWords, message, randomWord, currentWord, cols} from "../../stores";
	...
	const share = () => {
		...
		navigator.clipboard.writeText(
			`${$randomWord.join('')} kelimesini bir türlü bilemedim :/\n\n` +
			scoreBoard.join("\n")
        ...
        <div class="result">
            <div class="result-inner">
                <a class="share-btn" target="_blank"
                   href={`https://twitter.com/intent/tweet?text=${$randomWord.join('')} kelimesini bir türlü bilemedim :/%0a%0a${scoreBoard.join("%0a")}`}>
                    TWEET AT
                </a>
